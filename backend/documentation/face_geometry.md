...
| :--- | :--- | :--- | :--- |
| **`face_crop.jpg`** | Normalized biometric face crop | Standardized $380 \times 380$ px anchor | Ground-truth reference for interactive A/B workbench comparisons |
...
All visual artifacts generated by this module are persisted using `save_optimized_image` from [`backend/pipeline/image_utils.py`](../pipeline/image_utils.py). Dimensions exceeding 1080p are downscaled via area interpolation (`cv2.INTER_AREA`) and compressed at $Q=80$, slashing payload size by $>85\%$ for fast browser rendering.
